...
SD.prototype={
	...
	set3x3:function(i,j){ //为对角线上的三个三宫格随机生成。
	...
	createDoms:function(){ //生成9*9。
		...
		for(var k=0;k<9;k++){
			...
			$(".sdli:eq("+k+") .sdspan").eq(0).addClass('nl');
			$(".sdli:eq("+k+") .sdspan").eq(8).addClass('nr');
		}
		$(".sdli:eq(2) .sdspan,.sdli:eq(5) .sdspan").addClass('bb');
		$(".sdli:eq(3) .sdspan,.sdli:eq(6) .sdspan").addClass('bt');
		$(".sdli:eq(0) .sdspan").addClass('nt');
		$(".sdli:eq(8) .sdspan").addClass('nb');

		html = 
		'<div class="dashboard">'+
			'<input type="image" id="restart" onclick="sd.checkRes();" img src="./images/check.png"></input>&emsp;'+
			'<input type="image" id="restart" onclick="javascript:window.location.reload();" img src="./images/refresh.png"></input>'+
		'</div>'
		$(".container").prepend(html);
